    ...
    public function tree()
    {
        $query = ContentTree::findBySql("
            SELECT `content_tree`.`id`,
                   ...
                   `content_tree`.`depth`,
                   `content_tree`.`hide`,
                   IFNULL(ct.alias_path, IFNULL(ctt.alias_path, (SELECT alias_path FROM content_tree_translation WHERE content_tree_id = IFNULL(`content_tree`.link_id, `content_tree`.id) LIMIT 1))) AS `alias_Path`,
